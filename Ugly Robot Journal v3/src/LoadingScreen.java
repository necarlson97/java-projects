...
public class LoadingScreen extends JournalTextWindow{
	...
	Color startingColor = Color.pink;
	Color transitionColor = startingColor;
	
	public LoadingScreen(){
		titleFont = Journal.titleFont;
	}
	
	public Color overlayColor(Color eventualColor) {
		
		float red = (float) eventualColor.getRed()/255;
		float grn = (float) eventualColor.getGreen()/255;
		float blue = (float) eventualColor.getBlue()/255;
		float loadingRatio = (float) (loading-30) / (loadingMax-30);
		
		return new Color(red, grn, blue, loadingRatio);
	...
	public void run() {
		...
		if(loading > 30) {
			if(Journal.sidebar == null) Journal.sidebar = new DateSidebar();
			transitionColor = overlayColor(Journal.sidebar.getDayColor());
		}
	...
	private void renderHelpText(Graphics g) {
		...
		g.drawString("Ugly Robot Journal v3", margin*30, shadowY);
		
		int textY = (int) (margin*(15-Journal.oscillateNumber(count, 60, .8)));
		g.setColor(startingColor);
		g.drawString("Ugly Robot Journal v3", margin*30, textY);
		g.setColor(transitionColor);
		g.drawString("Ugly Robot Journal v3", margin*30, textY);
