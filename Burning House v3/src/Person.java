...
public abstract class Person extends GameObject {
	...
	public void run() {
		...
		if(alive) {
			...
			else if(y < groundY && fallDist == 0) fallDist = (int) (groundY-y);
	...
	public GameObject grab(Person grabber) {
		...
		if(!Driver.player.followTrain.contains(this))Driver.player.followTrain.add(this);
	...
	public void release() {
		if(!holder.down) {
			down = true;
			Driver.player.followTrain.remove(this);
	...
	public Stair closestStair() {
		Story checkStory = Driver.house.stories[0];
		if(module != null) checkStory = module.room.story;
		Stair closest = checkStory.stairs[0];
		for(int i=1; i<checkStory.stairs.length; i++) {
			if( closest == null || 
					(checkStory.stairs[i] != null && Math.abs(closest.x-x) > Math.abs(checkStory.stairs[i].x-x)) )
				closest = checkStory.stairs[i];
		}
		return closest;
	}
	...
	public void climbStairs(Stair s) {
		if(s != null) {
			if(s.x < x - width/2) goLeft();
			else if(s.x > x + width/2) goRight();
			else if(s.y > y && s.stairAbove != null) {
				x = s.x;
				y = s.y;
			}
			else if(s.y < y && s.stairBelow != null) {
				x = s.x;
				y = s.y;
			}
		}
