...
public class Stairs {
	...
	int width = BurningHouse.player.width + 4;
	int height =  BurningHouse.player.height + 4;
	...
	public Stairs(Floor up, Floor down) {
		x = (r.nextInt(up.modules.length)*up.indexWidth)
				+up.indexWidth/2+BurningHouse.leftOffset;
		upY = up.floorY;
		downY = down.floorY;
	}
	
	public Stairs(Floor up, Floor down, int moduleIndex) {
		x = (moduleIndex*up.indexWidth)
				+up.indexWidth/2+BurningHouse.leftOffset;
		upY = up.floorY;
		downY = down.floorY;
	}
